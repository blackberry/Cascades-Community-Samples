        ...
        QObject(parent)
{
    ...
    qDebug() << "GGGG does the database file already exist at " << _source;
    if (!databaseExists()) {
        qDebug() << "GGGG no, so let's create it";
        createDatabase();
    } else {
        qDebug() << "GGGG database already exists";
    }
    if (!database.isOpen()) {
        bool ok = database.open();
        qDebug() << "GGGG user database opened " << ok;
        if (!ok) {
            const QSqlError error = database.lastError();
            qDebug() << "GGGG result=" << error;
            return;
        }
    } else {
        qDebug() << "GGGG database is already open";
    }
    ...
    qDebug() << "GGGG loadRequestData signal connected=" << conn_result;

    conn_result = connect(this, SIGNAL(signalLoadTaskData()), this, SLOT(onLoadTaskData()), Qt::QueuedConnection);
    qDebug() << "GGGG loadTaskData signal connected=" << conn_result;
...
bool UserSqlDataSource::databaseExists()
{
    ...
    qDebug() << "GGGG databaseExists: " << filePath;
    QFile file(filePath);
    return file.exists();
}
...
void UserSqlDataSource::createDatabase()
{
    ...
    qDebug() << "GGGG creating database";

    if (_sda != 0) {
        qDebug() << "GGGG closing old database connection";
    ...
    qDebug() << "GGGG database file deleted=" << deleted;

    QFile file(filePath);

    qDebug() << "GGGG setting database name=" << filePath;
    database.setDatabaseName(filePath);

    bool database_opened = database.open();
    qDebug() << "GGGG createDatabase(): database opened " << database_opened;

    if (database_opened) {

        if (file.open(QIODevice::ReadWrite)) {
            qDebug() << "GGGG creating table annual_leave_requests";
            ...
            qDebug() << "GGGG result=" << error2;

            qDebug() << "GGGG creating table annual_entitlement";
            ...
            const QSqlError error3 = database.lastError();
            qDebug() << "GGGG result=" << error3;
            ...
            const QSqlError error = database.lastError();
            qDebug() << "GGGG failed to create database. error=" << error;
        }
    } else {
        const QSqlError error = database.lastError();
        qDebug() << "GGGG Don't have read/write access to db file. error=" << error;
...
{
    qDebug() << "GGGG reset is recreating the database";
    createDatabase();
}
...
void UserSqlDataSource::loadRequestData(QString leave_year)
{
    ...
    qDebug() << "GGGG loadRequestData(" << leave_year << ")";

    // The SQLite date() function returns the date in this format: YYYY-MM-DD
    _query = QString("select local_request_id, leave_year, date(from_date_s, 'unixepoch' , 'utc') as 'from_date', ");
    _query.append("date(to_date_s, 'unixepoch', 'utc') as 'to_date', half_days, booking_status, last_op_status, note, updating_leave_year, ");
    _query.append("date(updating_from_date_s, 'unixepoch' , 'utc') as 'updating_from_date', date(updating_to_date_s, 'unixepoch' , 'utc') as 'updating_to_date',");
    ...
    qDebug() << "GGGG _query=" << _query;
    qDebug() << "GGGG emitting loadRequestData() signal";
...
void UserSqlDataSource::onLoadRequestData(const QString& sql)
{
    ...
    qDebug() << "GGGG onLoadRequestData: " << sql;
    QVariant list = _sda->execute(sql);

    _requests_model_list = list.value<QVariantList>();
    qDebug() << "GGGG about to emit signalRequestsLoaded: " << _requests_model_list;
...
{
    qDebug() << "GGGG onLoadTaskData";

    // The SQLite date() function returns the date in this format: YYYY-MM-DD
    QString sql("select task_id, task_type, local_request_id, decision, user_id, leave_year, date(from_date_s, 'unixepoch' , 'utc') as 'from_date', ");
    sql.append("date(to_date_s, 'unixepoch', 'utc') as 'to_date', half_days, task_status, note,");
    sql.append("first_day_half, last_day_half, day_half, date(decision_date_s, 'unixepoch' , 'utc') as 'decision_date',");
    sql.append("updating_leave_year, date(updating_from_date_s, 'unixepoch' , 'utc') as 'updating_from_date', date(updating_to_date_s, 'unixepoch', 'utc') as 'updating_to_date',");
    ...
    qDebug() << "GGGG about to emit tasksLoaded: " << _tasks_model_list;
...
int UserSqlDataSource::getCurrentStatus(int local_request_id)
{
    ...
    qDebug() << "GGGG getCurrentStatus:" << select_sql;
...
int UserSqlDataSource::getLastOpStatus(int local_request_id)
{
    ...
    select_sql = select_sql.arg(local_request_id);
    qDebug() << "GGGG getLastOpStatus:" << select_sql;
    QVariant result = _sda->execute(select_sql);
    QVariantList list = result.value<QVariantList>();
    if (list.size() > 0) {
        QVariant row = list.first();
...
void UserSqlDataSource::updateEntitlement(int leave_year, int total_entitlement_half_days, int current_entitlement_half_days)
{
    ...
    qDebug() << "GGGG updateEntitlement:" << insert_sql;
...
Entitlement UserSqlDataSource::getEntitlement(int leave_year)
{
    ...
    qDebug() << "GGGG getEntitlement:" << select_sql;
    QVariant result = _sda->execute(select_sql);
    QVariantList list = result.value<QVariantList>();
    if (list.size() > 0) {
        QVariant row = list.first();
        ...
        qDebug() << "GGGG no annual_entitlement data for " << leave_year;
...
int UserSqlDataSource::storeRequest(int fdate, int tdate, QString note, int leave_year, bool first_day_half, bool last_day_half, int day_half)
{
    qDebug()
            << QString("GGGG storeRequest: from_date=%1 to_date=%2 note=%3 first_day_half=%4 last_day_half=%5 day_half=%6").arg(fdate).arg(tdate).arg(note).arg(first_day_half).arg(last_day_half).arg(
                    day_half);
    ...
    insert_sql = insert_sql.arg(leave_year).arg(fdate).arg(tdate).arg(-1).arg(STATUS_BOOKING_PENDING).arg(note.toAscii().constData()).arg(fdh).arg(ldh).arg(day_half);
    qDebug() << "GGGG storeRequest:" << insert_sql;
    _sda->execute(insert_sql);
...
{
    qDebug()
            << QString("GGGG updateRequest: local_request_id=%4 originating_op_id=%6 leave_year=%5 updating_from_date=%1 updating_to_date=%2 note=%3").arg(updating_from_date_s).arg(updating_to_date_s).arg(
    ...
    qDebug() << "GGGG updateRequest:" << update_sql;
...
{
    qDebug()
            << QString("GGGG applyBookingUpdate: local_request_id=%1").arg(local_request_id);
    ...
    qDebug() << "GGGG applyBookingUpdate:" << update_sql;
    _sda->execute(update_sql);
...
{
    qDebug() << QString("GGGG countBookingUpdates: local_request_id=%1 originating_op_id=%2").arg(local_request_id).arg(originating_op_id);
    int count = -1;
...
{
    qDebug() << QString("GGGG countTasks");
    int count = -1;
...
DbAnnualLeaveRecord UserSqlDataSource::getAnnualLeaveRequest(int local_request_id)
{
    ...
    select_sql = select_sql.arg(local_request_id);
    qDebug() << "GGGG getAnnualLeaveRequest:" << select_sql;
    QVariant result = _sda->execute(select_sql);
    QVariantList list = result.value<QVariantList>();
...
int UserSqlDataSource::getApprovalTaskYear(int task_id)
{
    ...
    qDebug() << "GGGG getApprovalTaskYear:" << select_sql;
    QVariant result = _sda->execute(select_sql);
    QVariantList list = result.value<QVariantList>();
...
{
    qDebug() << QString("GGGG updateHalfDays: local_request_id=%2 half_days=%1").arg(half_days).arg(local_request_id);
    ...
    qDebug() << "GGGG updateHalfDays:" << update_sql;
    _sda->execute(update_sql);
...
void UserSqlDataSource::updateRequestStatus(int local_request_id, int op_status)
{
    ...
    qDebug() << "GGGG updateRequestStatus local_request_id=" << local_request_id << " op_status=" << op_status;
    ...
    switch (op_status) {
        ...
        case STATUS_UPDATE_REJECTED:
            ...
            break;
        default:
            qDebug() << "GGGG ERROR: unexpected op_status in updateRequestStatus:" << op_status;
    }

    qDebug() << "GGGG updateRequestStatus:" << update_sql;
    _sda->execute(update_sql);
...
{
    qDebug() << "GGGG updateLastOpStatus local_request_id=" << local_request_id << " op_status=" << op_status;
    ...
    qDebug() << "GGGG updateLastOpStatus:" << update_sql;
    _sda->execute(update_sql);
}

void UserSqlDataSource::setOriginatingOpId(int local_request_id, int originating_op_id) {
    qDebug() << "GGGG setOriginatingOpId local_request_id=" << local_request_id << " originating_op_id=" << originating_op_id;
    ...
    qDebug() << "GGGG setOriginatingOpId:" << update_sql;
    _sda->execute(update_sql);
}

void UserSqlDataSource::clearProcessingStatus(int local_request_id, int leave_year)
{
    qDebug() << "GGGG clearProcessingStatus local_request_id=" << local_request_id;
    ...
    qDebug() << "GGGG clearProcessingStatus:" << update_sql;
    _sda->execute(update_sql);
    loadRequestData(QString::number(leave_year));
}
...
{
    qDebug() << QString("GGGG deleteApprovalTask: task_id=%1").arg(task_id);
    QString delete_sql("delete from approval_tasks where task_id=%1");
    delete_sql = delete_sql.arg(task_id);
    qDebug() << "GGGG deleteApprovalTask:" << delete_sql;
    _sda->execute(delete_sql);
...
int UserSqlDataSource::storeApprovalTask(QString user_id, int task_type, int fdate, int tdate, int leave_year, bool first_day_half, bool last_day_half, int day_half, int half_days,
        int local_request_id, QString note, int updating_fdate, int updating_tdate, int updating_leave_year, bool updating_first_day_half, bool updating_last_day_half, int updating_day_half, int updating_half_days, int originating_op_id)
{

    qDebug() << QString("GGGG storeApprovalTask: user_id=%1 local_request_id=%2 task_type=%3").arg(user_id).arg(local_request_id).arg(task_type);
    int fdh = Utilities::boolToInt(first_day_half);
    int ldh = Utilities::boolToInt(last_day_half);
    ...
    insert_sql.append("task_type, updating_leave_year, updating_from_date_s, updating_to_date_s, updating_first_day_half, updating_last_day_half, updating_day_half, updating_half_days, originating_op_id) ");
    ...
    insert_sql = insert_sql.arg(user_id).arg(leave_year).arg(fdate).arg(tdate).arg(fdh).arg(ldh).arg(day_half).arg(half_days).arg(TASK_STATUS_AWAITING_MANAGER).arg(local_request_id).arg(
            note.toAscii().constData()).arg(task_type).arg(updating_leave_year).arg(updating_fdate).arg(updating_tdate).arg(ufdh).arg(uldh).arg(updating_day_half).arg(updating_half_days).arg(originating_op_id);
    qDebug() << "GGGG storeApprovalTask:" << insert_sql;
    _sda->execute(insert_sql);
...
{
    qDebug() << QString("GGGG updateApprovalTask: task_id=%1 decision=%2").arg(task_id).arg(decision);
    ...
    qDebug() << "GGGG updateApprovalTask:" << update_sql;
    _sda->execute(update_sql);
    loadTaskData();
}
...
{
    qDebug() << QString("GGGG updateApprovalTaskProcessingStatus: task_id=%1 decision=%2").arg(task_id).arg(task_status);
    ...
    qDebug() << "GGGG updateApprovalTaskProcessingStatus:" << update_sql;
    _sda->execute(update_sql);
    loadTaskData();
}
