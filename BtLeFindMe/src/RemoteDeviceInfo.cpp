...
void RemoteDeviceInfo::update(const QString &deviceAddress)
{
    ...
    qDebug() << "XXXX RemoteDeviceInfo::update : deviceAddress" << deviceAddress;
    ...
    qDebug() << "XXXX RemoteDeviceInfo::update: _name" << _name;
...
QString RemoteDeviceInfo::serviceDescription(const QString &uuid) {
	...
	qDebug() << "XXXX RemoteDeviceInfo::serviceDescription() - uuid=" << uuid;
	...
	if (rx.exactMatch(uuid)) {
		qDebug() << "XXXX RemoteDeviceInfo::serviceDescription() - uuid matched rx.cap(2)=" << rx.cap(2);
		tempUuid = rx.cap(2);
	} else {
		qDebug() << "XXXX RemoteDeviceInfo::serviceDescription() - uuid not matched=";
	}
