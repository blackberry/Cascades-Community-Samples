...
static char* CSC_SERVICE_UUID = "1816";
static const char* CSC_MEASUREMENT = "0x2A5B";
...
void notifications_cb(int instance, uint16_t handle, const uint8_t *val, uint16_t len, void *userData) {
	...
	if (handle == cscdc->getCscValueHandle()) {
		cscdc->addNotification(val, len);
		cscdc->logMostRecentNotification();
	}
...
void gatt_service_connected(const char *bdaddr, const char *service, int instance, int err, uint16_t connInt, uint16_t latency, uint16_t superTimeout, void *userData) {
	...
	bt_gatt_characteristic_t* csc_measurement_characteristic;

	csc_measurement_characteristic = (bt_gatt_characteristic_t*) malloc(sizeof(bt_gatt_characteristic_t));

	if (NULL == csc_measurement_characteristic) {
	...
	if (num_characteristics > -1) {
		...
		for (int i = 0; i < characteristicListSize; i++) {
			...
			if (strcmp(characteristicList[i].uuid, CSC_MEASUREMENT) == 0) {
				qDebug() << "YYYY CSC Measurement characteristic available";
				cscdc->setCscHandle(characteristicList[i].handle);
				cscdc->setCscValueHandle(characteristicList[i].value_handle);
//				// enable=1 switches the notification on for the specified characteristic
				qDebug() << "YYYY registering forcsc_measurement notification. uuid,handle,value_handle=" << characteristicList[i].uuid << "," << characteristicList[i].handle << "," << characteristicList[i].value_handle;
				errno= 0;
				...
				qDebug() << "YYYY rc from registering for CSC measurement notification=" << rc;
			} else {
			}
		}
...
//		/* END WORKAROUND */
		...
		qDebug() << "YYYY done registering for CSC measurement notifications";
	}
}
...
void BluetoothHandler::receiveCscNotifications() {
	...
	CscDataContainer* cscdc = CscDataContainer::getInstance();
	QString device_addr = cscdc->getCurrentDeviceAddr();
	QString device_name = cscdc->getCurrentDeviceName();
	cscdc->resetData();
	...
	while (!done_connecting) {
		if (bt_gatt_connect_service(device_addr.toAscii().constData(), CSC_SERVICE_UUID, NULL, &conParm, this) < 0) {
			qDebug() << "YYYY GATT connect service request failed: " + QString::number(errno) + " (" + QString(strerror(errno)) + ")";
			attempt_count++;
			if (attempt_count > 5) {
				done_connecting = true;
				qDebug() << "XXXX given up attempting to connect to GATT service";
			} else {
				delay(1);
				qDebug() << "XXXX trying again to connect to GATT service";
			}
		} else {
			qDebug() << "YYYY requested connection to CSC service OK";
			done_connecting = true;
		}
	}
...
void BluetoothHandler::setRemoteDevice(const QString &address) {
	...
	CscDataContainer::getInstance()->addCscDevice(_remoteDeviceInfo->property("name").toString().toLatin1().data(), _remoteDeviceInfo->property("address").toString().toLatin1().data(),
...
void BluetoothHandler::stopCscNotifications() {
	...
	if (bt_gatt_disconnect_service(device_addr.toAscii().constData(), CSC_SERVICE_UUID) < 0) {
		qDebug() << "YYYY GATT disconnect service request failed: " + QString::number(errno) + " (" + QString(strerror(errno)) + ")";
	} else {
		qDebug() << "YYYY disconnected from CSC service OK";
	}
