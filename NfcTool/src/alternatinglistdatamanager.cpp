...
AlternatingListDataManager::AlternatingListDataManager(const QString& qmlFile) : m_qmlFile(qmlFile) {}

AlternatingListDataManager::~AlternatingListDataManager() {}

bb::cascades::VisualNode* AlternatingListDataManager::createItem(bb::cascades::ListView* list, const QString &type)
{
    Q_UNUSED(list);

//    qDebug() << "In createItem()";
...
//	qDebug() << "Setting ordinal    : " << ordinal;
//	qDebug() << "Setting itemLabel  : " << itemLabel;
//	qDebug() << "Setting title      : " << title;
//	qDebug() << "Setting description: " << description;

	if (!((itemLabel.isEmpty() or itemLabel.isNull()) and
		  (title.isEmpty()or title.isNull()) and
		  (description.isEmpty() or description.isNull()))) {

//		qDebug() << "Non-empty entries so adding to list";
		...
		qDebug() << "Empty entries so not adding to list";
	}
}
