...
AlternatingListItem::AlternatingListItem(const QString& qmlFile) : CustomControl()
{
    setup(qmlFile);
}
...
void AlternatingListItem::setup(const QString& qmlFile)
{
	...
	QDeclarativeContext *derivedContext = new QDeclarativeContext(document->documentContext(), this);
