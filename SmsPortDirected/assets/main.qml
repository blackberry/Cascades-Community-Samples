...
Page {
    Container {
        id: mainPage
        objectName: "mainPage"
        signal sendMessage(string message, string address, int port)
        layout: StackLayout {
        }
        Label {
            id: lblHeading_log
            text: qsTr("Port Directed SMS Messages")
        ...
        Button {
            ...
            onClicked: {
                ...
                if (destAddr.text.length > 2) {
                    mainPage.sendMessage(sendText.text, destAddr.text, port.text);
        ...
        TextField {
            id: port
            text: "16000"
            hintText: "Port number to address the message to"
            verticalAlignment: VerticalAlignment.Center
            horizontalAlignment: HorizontalAlignment.Center
        }
        ImageView {
