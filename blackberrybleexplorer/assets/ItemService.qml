...
Container {
    ...
    property int service_identifier: -1
    ...
    function setOffset(offset) {
        ...
        console.log("QQQQ offset changed on ItemService " + service_identifier + " to " + centre_offset + " - translationX=" + translationX);
    }

    function disableTranslationAnimation() {
        console.log("QQQQ disabling translation animation for item " + service_identifier);
        x_controller.enabled = false;
    }

    function enableTranslationAnimation() {
        console.log("QQQQ enabling translation animation for item " + service_identifier);
    ...
    onDepthChanged: {
        scaleX = 2 - (depth * 0.25);
        scaleY = 2 - (depth * 0.25)
        console.log("QQQQ ItemService "+service_identifier+" depth changed to "+depth+", scale factors="+scaleX+","+scaleY);
    }

    onVisibleChanged: {
        console.log("CCCC visible changed in ItemService " + service_identifier + " to " + visible);
    }
